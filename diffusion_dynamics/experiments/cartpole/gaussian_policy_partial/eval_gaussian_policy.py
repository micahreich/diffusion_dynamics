...
if __name__ == "__main__":
    x0 = torch.tensor([0.0, torch.pi - 0.4, 0.0, -0.0], dtype=torch.float32)
    ...
    policy = ConditionalGaussianAgent.load_trained_model(
        '/home/dev/workspace/diffusion_dynamics/experiments/cartpole/gaussian_policy_partial/cartpole_gaussian_policy_partial1.pt'
    ).to(device)
    ...
    def u_policy(_t, observation):
        action_sample = policy.sample(observation.unsqueeze(0), deterministic=False)
        return action_sample[0, 0]

    test_i = 1
    obs = obs_fn_policy(test_i, x_hist_lqr)
    upolicy = policy.sample(obs.unsqueeze(0), deterministic=False)
