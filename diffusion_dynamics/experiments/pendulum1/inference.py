...
if __name__ == "__main__":
    trained_dataset_params = PendulumModel.load_trained_model("/workspace/diffusion_dynamics/experiments/pendulum1/saved_models/pendulum_model1_nocontrols")
    ...
    with torch.no_grad():
        ...
        for t in PendulumModel.scheduler.timesteps:
            sample[0, 0, 0:seq_len//2] = 0.3
            sample[0, 1, 0:seq_len//2] = 0.0
    ...
    ax[0].plot(ts_sim[:-1], xs_sim[:-1, 0], label=r"$\theta$", c="r", linestyle="--")
    ax[0].plot(ts_sim[:-1], xs_sim[:-1, 1], label=r"$\dot{\theta}$", c="b", linestyle="--")
    
    ax[0].plot(ts_sim[:-1], xs[:, 0], label=r"$\theta_{pred}$", c="r")
