...
if __name__ == '__main__':
    ...
    sin_data = 7.0 * torch.sin(x.unsqueeze(0) * freq.sample((n_samples, 1)) + freq.sample((n_samples, 1)))

    step_data = (
        torch.arange(seq_length).unsqueeze(0).expand(n_samples, seq_length)
        >= torch.randint(0, seq_length, (n_samples,)).unsqueeze(1)
    ).float()
