...
class UNet1D(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, base_channels=64, dim_mults=[1, 2, 4], attention=True):
        super().__init__()
            ...
            self.downs.append(
                nn.ModuleList(
                    [
                        ...
                        (
                            Residual(PreNorm(dim_out, LinearAttention(dim_out, heads=4, dim_head=32)))
                            if attention
                            else nn.Identity()
                        ),
        ...
        self.mid_attn = (
            Residual(PreNorm(mid_dim, LinearAttention(mid_dim, heads=4, dim_head=32))) if attention else nn.Identity()
        )
            ...
            self.ups.append(
                nn.ModuleList(
                    [
                        ...
                        (
                            Residual(PreNorm(dim_in, LinearAttention(dim_in, heads=4, dim_head=32)))
                            if attention
                            else nn.Identity()
                        ),
